...
function ModalContent({ title, children, doAfterClose }) {
  return (
    <>
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-4 text-gray-900 shadow">
        ...
        <Dialog.Close onClick={doAfterClose} className="relative float-right -top-24 -right-6 text-gray-400 hover:text-gray-500 rounded-full border border-black bg-gray-200">
          <TbX />
        </Dialog.Close>
        {children}
      </Dialog.Content>
